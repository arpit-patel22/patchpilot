...
export const DiagnoseSection = () => {
  ...
  const lines = [];
  lines.push(`PatchPilot Diagnosis — Ticket #${result.ticketId}`);
  lines.push(`Resolved in ${(elapsedMs / 1000).toFixed(1)}s`);
  lines.push("");

  if (tryThisFirst) {
    lines.push("TRY THIS FIRST:");
    lines.push(tryThisFirst);
    lines.push("");
  }

  causes.forEach((cause, i) => {
    lines.push(`#${String(i + 1).padStart(2, "0")}${i === 0 ? " · most likely" : ""} (${cause.probability}%) — ${cause.name}`);
    if (cause.explanation) lines.push(cause.explanation);
    if (cause.steps?.length) {
      lines.push("Fix steps:");
      cause.steps.forEach((step, j) => lines.push(`  ${j + 1}. ${step}`));
    }
    lines.push("");
  });

  lines.push("Generated by PatchPilot · patchpilot.online");
        ...
        <div className="diagnose-wrap" onKeyDown={handleKeyDown}>
          ...
          >
            <div className="pp-form-head">
              <div className="pp-form-title">New diagnosis</div>
              <span
                className="mono"
                style={{
                  fontSize: 11,
